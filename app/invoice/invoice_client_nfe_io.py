...
from app.invoice.constants.nfe_io_constants import OPTIONAL_FIELDS, VALID_BORROWER_TYPES, VALID_TAX_REGIMES, VALID_COUNTRIES_ISO_ALPHA3
...
class InvoiceClientNFEio(InvoiceClientInterface):
    ...
    def _borrower_validator(self, borrower: Dict[str, Any]) -> None:
        ...
        if not country or not isinstance(country, str):
            raise ValueError("O campo obrigatório 'borrower.address.country' está ausente ou inválido.")

        if country.upper() not in VALID_COUNTRIES_ISO_ALPHA3:
            raise ValueError(f"Sigla de país inválida: '{country}'. Esperado formato ISO 3166-1 alpha-3, ex: 'BRA', 'USA'.")

        borrower_type = borrower.get("type")
        if borrower_type and borrower_type not in VALID_BORROWER_TYPES:
            raise ValueError(f"'type' inválido em borrower: '{borrower_type}'. Valores válidos: {VALID_BORROWER_TYPES}")

        tax_regime = borrower.get("taxRegime")
        if tax_regime and tax_regime not in VALID_TAX_REGIMES:
            raise ValueError(f"'taxRegime' inválido em borrower: '{tax_regime}'. Valores válidos: {VALID_TAX_REGIMES}")
    ...
    def _process_optional_fields(self, data: Dict[str, Any], kwargs: Dict[str, Any]) -> None:
        """
        ...
        **kwargs
        ...
        for key in ["externalId", "issuedOn"]:
            if key not in kwargs:
                kwargs[key] = str(uuid4()) if key == "externalId" else datetime.utcnow().isoformat() + "Z"
